...
$(document).on('click', '.slide-group-edit', function(event) {
    var json = JSON.parse(atob($(this).attr('data-json')));
    $('#basic-modal-edit-group .title').val(json.title);
    $('#basic-modal-edit-group .id').val(json.id);
    $('#basic-modal-edit-group .slug').val(json.slug);
    $('#basic-modal-edit-group .description').val(json.description);
...
$(document).ready(function() {
    ...
    $(document).on('click', '.edit-group', function() {
        ...
        let formData = $('#editCategorySlide').serializeArray();
        let editorContent = CKEDITOR.instances['description'].getData();
        formData.push({ name: 'editor_description', value: editorContent });              
        $.ajax({
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
