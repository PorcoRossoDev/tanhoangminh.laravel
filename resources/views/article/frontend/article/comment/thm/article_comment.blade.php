        ...
        <div class="item flex gap-[20px] mb-[40px]">
            ...
            <div class="flex-1 bg-[#FFF6E4] px-[20px] py-[10px] rounded-[10px]">
                <div class="flex justify-between">
                    <div class="4xl:text-f24 text-f16 font-medium leading-[160%]">{{ $comment->fullname ?? 'anonymous' }}</div>
                    <div class="text-[#9397AD] text-f14 4xl:text-f24">{{ $comment->created_at->format('M d, Y') }}</div>
                </div>
                <div class="mt-[20px] 4xl:text-f24">
                    {{ $comment->message }}
                </div>
                <div class="flex items-center justify-end">
                    <button type="button" class="btn-like flex items-center justify-end {{ $comment->likes->isNotEmpty() ? 'like-active' : '' }}" data-id="{{ $comment->id }}" data-likes="{{ $comment->likes_count ?? 0 }}">
                        <svg class="4xl:w-[25px] 4xl:h-[25px]" viewBox="0 0 16 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <span class="4xl:text-f24 inline-block ml-1 text-[#9397AD] relative top-[1px] totalLikes">{{ $comment->likes_count ?? 0 }}</span>
                    </button>
                </div>
            </div>
        </div>
    @endforeach
    <div id="pagination" class="mt-4 pagenavi 4xl:text-f24">{!! $comments->links() !!}</div>
@endif