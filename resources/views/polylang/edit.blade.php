  ...
  ?>
  @if( env('APP_MULTIPE_LANGUAGE') == true )
              ...
              @if ($key != config('app.locale'))
                  ...
                  <div class="flex items-center mt-2">
                      <div class="w-9">
                          ...
                          <input type="text" name="language[{{ $key }}]" class="hidden"
                              value="{{ !empty($polylang[$key]) ? $polylang[$key] : '' }}">
                      </div>
                      <div class="flex-1 ml-2 flex items-center relative box-{{ $key }}">
                          ...
                          <input class="form-control w-full js_languageSearch" type="text"
                              value="{{ !empty($product->title) ? $product->title : '' }}"
                              data-language="{{ $key }}" data-module="{{ $module }}">
                          <ul class="absolute w-full top-full left-0 shadow-sm p-2 bg-white space-y-1 ulDropdown ulDropdown-{{ $key }} hidden"
                              style="top:100%;z-index: 99999;">
                          </ul>
                      </div>
                  </div>
              @endif
