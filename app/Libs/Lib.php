...
if (!function_exists('getAsideCatArticle')) {
    ...
    {
        return \App\Models\CategoryArticle::select('id', 'title', 'image', 'slug', 'image_json')
...
if (!function_exists('formatDateVietnamese')) {
    function formatDateVietnamese($dateString) {
        ...
        return date("d", strtotime($dateString)) . ' tháng ' . date("m", strtotime($dateString)) . ', ' . date("Y", strtotime($dateString));
    }
}
...
if (!function_exists('hasVideoExtension')) {
    function hasVideoExtension($url) {
        $videoExtensions = ['mp4', 'webm', 'mov', 'avi', 'flv', 'mkv'];
