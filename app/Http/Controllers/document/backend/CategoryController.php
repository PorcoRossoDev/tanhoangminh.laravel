...
class CategoryController extends Controller
{
    ...
    {
        $module = $this->table;
        $data =  CategoryDocument::where('alanguage', config('app.locale'))->with('user')->with('listDocument')->orderBy('lft', 'ASC');
        ...
        return view('document.backend.category.index', compact('data', 'module', 'configIs'));
    }
    public function create()
    {
        ...
        return view('document.backend.category.create', compact('module', 'htmlCatalogue', 'field'));
    }
    public function store(Request $request)
    {
        ...
        return redirect()->route('category_documents.index')->with('success', "Thêm mới danh mục thành công");
    }
    public function edit($id)
    {
        $module = $this->table;
        $detail  = CategoryDocument::where('alanguage', config('app.locale'))->find($id);
        if (!isset($detail)) {
            return redirect()->route('category_documents.index')->with('error', "Danh mục không tồn tại");
        }
        $htmlCatalogue = $this->nestedsetbie->dropdown([], config('app.locale'));
        $field = \App\Models\ConfigColum::where(['trash' => 0, 'publish' => 0, 'module' => $module])->get();
        return view('document.backend.category.edit', compact('detail', 'htmlCatalogue', 'module', 'field'));
    }
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('category_documents.index')->with('success', "Cập nhập danh mục thành công");
    }
    public function submit($request = [], $action = '', $id = 0, $arrayImg = [])
    {
        ...
        if ($action == 'create') {
            $id = CategoryDocument::insertGetId($_data);
        } else {
            CategoryDocument::find($id)->update($_data);
        }
        if (!empty($id)) {
            //xóa khi cập nhập
            if ($action == 'update') {
