...
class ContactController extends Controller
{
    ...
    {
        $data = Contact::where('type', 'expert')->orderBy('id', 'DESC');
        if (is($request->keyword)) {
            $data =  $data->where(function ($query) use ($request) {
                $query->where('email', 'like', '%' . $request->keyword . '%')
                    ->orWhere('fullname', 'like', '%' . $request->keyword . '%')
        ...
        $countContact = Contact::where('type', 'expert')->count();
        $countEmail = Contact::where('type', 'expert')->count();
        ...
        $module =  'contacts';
        return view('contact.backend.expert', compact('module', 'data', 'countContact', 'countEmail', 'temp_email'));
    }
