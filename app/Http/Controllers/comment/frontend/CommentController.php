...
class CommentController extends Controller
{
    ...
    public function listComment(Request $request)
    {
        ...
        $comments = Comment::where(['module_id' => $article_id])
        ->with(['likes' => function($q) use ($customerId) {
            $q->where('customer_id', $customerId);
        }])
        ->with('customer')->withCount('likes')->paginate(5);

        return view('article.frontend.article.comment.thm.article_comment', compact('comments'))->render();
    }
