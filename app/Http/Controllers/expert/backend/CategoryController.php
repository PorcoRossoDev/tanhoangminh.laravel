...
class CategoryController extends Controller
{
    ...
    protected $Polylang;
    protected $table = 'category_experts';
    ...
    {
        $module = $this->table;
        $data =  CategoryExpert::where('alanguage', config('app.locale'))->with('user')->with('listexpert')->orderBy('lft', 'ASC');
        ...
        return view('expert.backend.category.index', compact('data', 'module', 'configIs'));
    }
    public function create()
    {
        ...
        return view('expert.backend.category.create', compact('module', 'htmlCatalogue', 'field'));
    }
    public function store(Request $request)
    {
        ...
        return redirect()->route('category_experts.index')->with('success', "Thêm mới danh mục thành công");
    }
    public function edit($id)
    {
        $module = $this->table;
        $detail  = CategoryExpert::where('alanguage', config('app.locale'))->find($id);
        if (!isset($detail)) {
            return redirect()->route('category_experts.index')->with('error', "Danh mục không tồn tại");
        }
        $htmlCatalogue = $this->nestedsetbie->dropdown([], config('app.locale'));
        $field = \App\Models\ConfigColum::where(['trash' => 0, 'publish' => 0, 'module' => $module])->get();
        return view('expert.backend.category.edit', compact('detail', 'htmlCatalogue', 'module', 'field'));
    }
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('category_experts.index')->with('success', "Cập nhập danh mục thành công");
    }
    public function submit($request = [], $action = '', $id = 0, $arrayImg = [])
    {
        $category = CategoryExpert::find($id);
        if ($action == 'create') {
            $time = 'created_at';
            $user = 'userid_created';
        } else {
        ...
        if ($action == 'create') {
            $id = CategoryExpert::insertGetId($_data);
        } else {
            CategoryExpert::find($id)->update($_data);
        }
        if (!empty($id)) {
            //xóa khi cập nhập
            if ($action == 'update') {
                ...
                if( $category->parentid == 0 && $request['parentid'] > 0 ){
                    ...
                    $category_child = CategoryExpert::where(['parentid' => $category->id, 'alanguage' => config('app.locale')])->get();
                    //dd($category_child);
