...
class CategoryController extends Controller
{
    ...
    {
        $module = $this->table;
        $data =  CategoryCourse::where('alanguage', config('app.locale'))->with('user')->with('listCource')->orderBy('lft', 'ASC');
        ...
        return view('course.backend.category.index', compact('data', 'module', 'configIs'));
    }
    public function create()
    {
        ...
        return view('course.backend.category.create', compact('module', 'htmlCatalogue', 'field'));
    }
    public function store(Request $request)
    {
        ...
        return redirect()->route('category_courses.index')->with('success', "Thêm mới danh mục thành công");
    }
    public function edit($id)
    {
        $module = $this->table;
        $detail  = CategoryCourse::where('alanguage', config('app.locale'))->find($id);
        if (!isset($detail)) {
            return redirect()->route('category_courses.index')->with('error', "Danh mục không tồn tại");
        }
        $htmlCatalogue = $this->nestedsetbie->dropdown([], config('app.locale'));
        $field = \App\Models\ConfigColum::where(['trash' => 0, 'publish' => 0, 'module' => $module])->get();
        return view('course.backend.category.edit', compact('detail', 'htmlCatalogue', 'module', 'field'));
    }
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('category_courses.index')->with('success', "Cập nhập danh mục thành công");
    }
    public function submit($request = [], $action = '', $id = 0, $arrayImg = [])
    {
        ...
        if ($action == 'create') {
            $id = CategoryCourse::insertGetId($_data);
        } else {
            CategoryCourse::find($id)->update($_data);
        }
        if (!empty($id)) {
            //xóa khi cập nhập
            if ($action == 'update') {
                ...
                if( $category->parentid == 0 && $request['parentid'] > 0 ){
                    ...
                    $category_child = CategoryCourse::where(['parentid' => $category->id, 'alanguage' => config('app.locale')])->get();
                    //dd($category_child);
