...
class ArticleController extends Controller
{
    ...
    public function index($slug = "")
    {
        ...
        $slug = end($segments);
        $detail = Article::where(['slug' => $slug, 'alanguage' => config('app.locale'), 'publish' => 0])
            ->with('catalogues')
            ->withCount('comments')
            ->first();
        ...
        //bài viết liên quan
        $sameArticle = Article::select('articles.*')  // chọn tất cả cột từ articles
        ->join('catalogues_relationships', 'articles.id', '=', 'catalogues_relationships.moduleid')
        ->where('articles.alanguage', config('app.locale'))
        ->where('catalogues_relationships.catalogueid', $catalogues->id)
        ->where('catalogues_relationships.moduleid', '!=', $detail['id'])
        ->where('articles.publish', 0)
        ->where('catalogues_relationships.module', 'articles')
        ->groupBy('catalogues_relationships.moduleid')
        ->orderBy('articles.order', 'ASC')
        ->orderBy('articles.id', 'DESC')
        ->limit(4)
        ->get();

        $sameArticle->load('catalogues');
        ...
        return view('article.frontend.article.index', compact('module', 'fcSystem', 'detail', 'seo', 'breadcrumb', 'sameArticle', 'catalogues', 'comment_view'));
    }
}
