    ...
    public function register_store(Request  $request)
    {
        ...
        if (config('app.locale') == 'vi') {
            $request->validate([
                ...
                'name.required' => '全名是必填字段.',
                'email.required' => '电子邮件为必填字段.',
                ...
                'password.min' => '密码长度必须至少为 6 个字符.',
                'confirm_password.min' => '重新输入密码必须至少包含 6 个字符.',
                'confirm_password.required' => '重新输入密码是必填字段.',
                'confirm_password.same' => '重新输入密码与密码不匹配.',
            ]);
        } else {
            $request->validate([
                'name' => 'required',
        ...
        return redirect()->route('customer.login')->with('success', '会员注册成功');
    }
    //quên mật khẩu
