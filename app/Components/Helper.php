...
class Helper
{
    ...
    //thêm vào bảng cources_relationship
    function courses_relation_ship($productid = 0, $catalogueid = 0, $tmp_catalogue = [], $module = '')
    {
        $_cource_relation_ship = [];
        if (isset($tmp_catalogue)) {
            foreach ($tmp_catalogue as $v) {
                $_cource_relation_ship[] = array(
                    'module' => $module,
                    'moduleid' => $productid,
                    'catalogueid' => $v,
                    'created_at' => Carbon::now(),
        ...
        DB::table('course_relationships')->insert($_cource_relation_ship);
    }
