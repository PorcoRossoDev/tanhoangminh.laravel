...
use App\Http\Controllers\course\frontend\CategoryController as CategoryControllerCourse;
use Session;
            ...
            if (!empty($segments)) {
                ...
                if (!empty($checkURL)) {
                    if (in_array($first, config('app.alt_langs'))) {
                        ...
                        if ($checkURL->module == 'category_courses') {
                            Route::get('/' . $first . '/{slug}', [CategoryControllerCourse::class, 'index'])->middleware(['web', 'locale'])->where(['slug' => '.+'])->name('routerURL');
                        }
                        ...
                        if ($checkURL->module == 'category_courses') {
                            Route::get('/{slug}', [CategoryControllerCourse::class, 'index'])->middleware(['web', 'locale'])->where(['slug' => '.+'])->name('routerURL');
                        }
                        if ($checkURL->module == 'courses') {
